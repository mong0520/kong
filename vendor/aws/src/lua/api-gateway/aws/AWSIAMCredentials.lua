...
function AWSIAMCredentials:fetchIamUser()
    ngx.log(ngx.WARN, "Fetching IAM User from:",
    ...
    ngx.log(ngx.DEBUG, string.format("Fetching IAM User result: status %s", status))
...
function AWSIAMCredentials:fetchSecurityCredentialsFromAWS()
    local iamURL = ""
    local aws_config_str = ngx.shared.kong:get("aws_config")
    ngx.log(ngx.INFO, "aws config str is:", aws_config_str)
    local aws_config = cjson.decode(aws_config_str)
    local security_credentials_url = aws_config.security_credentials_url
    ngx.log(ngx.DEBUG, "security credentials url is:", security_credentials_url)
    if security_credentials_url == nil or security_credentials_url == "" then
        iamURL = self.security_credentials_url .. self:getIamUser() .. "?DurationSeconds=" .. self.security_credentials_timeout
        ...
        ngx.log(ngx.DEBUG, "IAM Request: ", iamURL)
        ...
        ngx.log(ngx.INFO, "fetch iam user by task-iam-roles")
        self.security_credentials_host = "169.254.170.2"
        self.security_credentials_url = security_credentials_url
        local iam = self:fetchIamUser() -- fetch latest iam credentials
        if (iam ~= nil and iam ~= "") then
            local iam_response = cjson.decode(iam)
            cache.AccessKeyId = iam_response["AccessKeyId"]
            cache.SecretAccessKey = iam_response["SecretAccessKey"]
            cache.Token = iam_response["Token"]
            cache.ExpireAt = iam_response["Expiration"]
            cache.ExpireAtTimestamp = awsDate.convertDateStringToTimestamp(cache.ExpireAt, true)
            if (cache.ExpireAtTimestamp - os.time() > 0) then
                iamCache:put("iam_credentials", cjson.encode(cache))
            end
            ngx.log(ngx.DEBUG, "Fetch latest iam credentials success.")
            return true
        end
