...
local function get_hashed_canonical_request(method, uri, querystring, headers, requestPayload)
    ...
    for _, v in ipairs(headers) do
        ...
        canonicalHeaders = canonicalHeaders .. v["name"] .. ":" .. v["value"] .. "\n"
        signedHeaders = signedHeaders .. v["name"] .. ";"
    end
    --remove the last ";" from the signedHeaders
    signedHeaders = string.sub(signedHeaders, 1, -2)

    -- workaround: we need to make sure the header sequence be "host;x-amz-date"
    -- because HmacAuthV4Handler:getAuthorizationHeader hard coded it.
...
function HmacAuthV4Handler:getSignature(http_method, request_uri, uri_arg_table, request_payload )
    ...
    local headers = {}
    headers[1] = { name = "host", value = self.aws_service .. "." .. self.aws_region .. ".amazonaws.com"}
    headers[2] = { name = "x-amz-date", value = date2}
