...
local function debug(...)
    if debug_mode then
        ...
        -- print(ngx.DEBUG, "HttpClient.Http: ", ...)
    end
end
...
function _M.request(self, reqt)
    ...
    if nreqt.scheme == "https" then
        debug("https request, wrap it with ssl")
        local params = {
          mode = "client",
          protocol = "any",
          verify = "none",
        ...
        if err then
            debug("failed to wrap ssl: " .. tableToString(err) .. type(err), err)
        end
        debug("ssl wrapped")

        if reqt.ssl_verify then
            params.verify = "peer"
        end
        local ok, err = sock:dohandshake()
        if not ok then
            debug("handshare failed" .. err .. type(err))
        end
        debug("ssl handshake finished")
